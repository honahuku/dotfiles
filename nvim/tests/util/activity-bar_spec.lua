...
describe("activity-bar", function()
  ...
  describe("panel_index_for_line", function()
    it("各パネルの表示行からパネルのインデックスを返す", function()
      assert.are.same(1, activity_bar.panel_index_for_line(1))
      assert.are.same(1, activity_bar.panel_index_for_line(activity_bar.ROWS_PER_PANEL))
      assert.are.same(
        #activity_bar.PANELS,
        activity_bar.panel_index_for_line(#activity_bar.PANELS * activity_bar.ROWS_PER_PANEL)
      )
    end)
  ...
  describe("render_lines", function()
    it("各パネルを3行のアイコン付き文字列として返す", function()
